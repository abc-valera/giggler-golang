...
func GetEnvUrl(key string) *url.URL {
	return ParseUrl(GetEnv(key))
}

// GetEnvUint gets the env var and parses it as uint.
// Panics if the env var is not set or has invalid value.
func GetEnvUint(key string) uint {
	val, err := strconv.ParseUint(GetEnv(key), 10, 32)
	if err != nil {
		panic("failed to parse " + key + " env var as uint")
	}
	return uint(val)
}
