...
func initCommand() func(ds.IDS) iCommand {
	switch ds.DbVal {
	case ds.DbVariantGorm:
		return func(dataStore ds.IDS) iCommand {
			return data.NewGormGenericCommand(ds.GormDS(dataStore), userModel.NewGormDTO)
		}
	default:
...
func Create(ctx context.Context, ds ds.IDS, req CreateReq) (userModel.User, error) {
	var span trace.Span
...
func Update(ctx context.Context, ds ds.IDS, req UpdateReq) (userModel.User, error) {
	var span trace.Span
	ctx, span = otel.Trace(ctx)
	defer span.End()
...
func Delete(ctx context.Context, ds ds.IDS, req DeleteReq) error {
	var span trace.Span
	ctx, span = otel.Trace(ctx)
	defer span.End()
